...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-metrics"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("POST /api/metrics",s.handleRecord)
    s.mux.HandleFunc("GET /api/metrics",s.handleLatest)
    s.mux.HandleFunc("GET /api/metrics/history",s.handleHistory)
